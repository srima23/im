...
public class CycleService {
    ...
    public int returnCycle(long id, int count) {
        var cycle = findByIdOrThrow404(id);

        // Get the current borrowed count
        int currentBorrowed = cycle.getNumBorrowed();

        if (count > currentBorrowed) {
            return -1;
        }
        int numBorrowed = currentBorrowed - count;
        cycle.setNumBorrowed(numBorrowed);
        cycleRepository.save(cycle);
        return count;
    }
